...
root.title("CAFE BILLING MANAGEMENT SYSTEM")
...
def Ref():
    try:
        # Validate and convert the input fields to float, handle empty or invalid input
        cof = float(Fries.get()) if Fries.get() else 0.0
        colfries = float(Largefries.get()) if Largefries.get() else 0.0
        cob = float(Burger.get()) if Burger.get() else 0.0
        cofi = float(Filet.get()) if Filet.get() else 0.0
        cochee = float(Cheese_burger.get()) if Cheese_burger.get() else 0.0
        codr = float(Drinks.get()) if Drinks.get() else 0.0
    except ValueError:
        # If there's an invalid input that can't be converted to float, show an error message
        messagebox.showerror("Invalid input", "Please enter valid quantities for all items.")
        return

    # Now you can proceed with the calculations, as all values will be numbers
    costoffries = cof * 25
